...
function runBuild(serverUrl, data, registry, matrixId) {
  ...
  server.on('connect', function () {
    ...
    server.emit('matrixStarted', data, matrixId);

    git.pullOrClone(data.repoUrl, function (err, repoPath) {
      if (err) {
        return server.emit('matrixError', err, matrixId);
      }

      builder.startBuild(data, repoPath, server, matrixId, function (err) {
        if (err) {
          server.emit('matrixError', err, matrixId);
        }

        server.emit('matrixCompleted', matrixId);
