...
function registry(options) {
  ...
  function notifyAgentRegistration(agent, registration) {
    ...
    clients.emit('agentRegistration', agent.id, registration);
  ...
  var clients = io.of('/client')
      .on('connection', function (client) {
        client.emit('repositories', repositories);
      });

  var agents = io.of('/agent')
      .on('connection', function (agent) {
        agent.on('register', function (registration) {
          if (Object.keys(repositories).indexOf(registration.repoUrl) == -1) {
            repositories[registration.repoUrl] = {};
          }

          if(!repositories[registration.repoUrl][agent.id]) {
            repositories[registration.repoUrl][agent.id] = [];
          }
          ...
          if (!registration.requirement) {
            ...
            repositories[registration.repoUrl][agent.id].push(registration.requirement);
