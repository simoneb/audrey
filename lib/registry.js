...
function registry(options) {
  ...
  io.of('/server')
      .on('connection', function (server) {
        ...
        server.on('run', function (data, buildId, cellIndex) {
          ...
          async.detect(agentIds, function (agentId, callback) {
              ...
              function (agentId) {
                if (!agentId) {
                  server.emit('noAgents', buildId, cellIndex);
                } else {
                  server.emit('agents', buildId, cellIndex);
                  agents[agentId].socket.emit('run', data, buildId, cellIndex);
                }
              });
        });
      });
